...
const vConnector_1 = require("../visualElements/vConnector");
...
class Canvas {
    ...
    static showGrid;
    static observers;
    static _canvasBeingTransformed;
    static newOrigin;
    static valuesEl;
    static legendEl;
    ...
    static subscribe(obj) {
        if (obj instanceof vEdge_1.VEdge) {
            ...
            let edgeInList = edgeFactory_1.EdgeFactory.contains(vEdges, obj);
    ...
    static reset() {
        ...
        const vNodeObservers = Canvas.observers.filter((observer) => observer instanceof vNode_1.VNode);
     ...
     * @param {Vector} pos
     */
    static displayValues(pos, __UNUSED_ARG__) {
        // **** Legends
        if (!this.valuesEl) {
     ...
     * @param {Vector} pos
     */
    static showLegend(pos, __UNUSED_ARG__) {
        if (!this.legendEl) {
    ...
    static mWheel(evt) {
        ...
        // Amount to scale.
        const amnt = evt.deltaY < 0 ? 1.02 : 0.98;

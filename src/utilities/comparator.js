...
class Comparator {
     ...
     */
    static compareValue(item, other) {
        if (item - other > 0) {
            return 1;
        }
     ...
     */
    static compareLength(item, other) {
     ...
     */
    static compareAlphabetically(word, other) {
    ...
    static staticMethodNames = Object.getOwnPropertyNames(Comparator).filter(name => typeof Comparator[name] === "function" && name !== "length" && name !== "prototype" && name !== "name");
}

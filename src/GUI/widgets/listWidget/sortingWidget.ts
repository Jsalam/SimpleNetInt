...
export class SortingWidget {
  ...
  updateVisuals() {
    ...
    let tmpElement = DOM.elements.sortingWidgets.querySelectorAll(
      "#" + this.id,
    )[0] as HTMLElement;
   ...
   * @returns
   */
  private getSortingAttributesFromVNodes(): string[] {
    ...
    for (let i = 0; i < vNodesTemp.length; i++) {
      ...
      try {
        ...
        if (attrib) Utilities.traverse(attrib as Object, cllbck, path);
        else
          console.warn(
            "node attributes do not have 'attAll' property. Nodes might not belong to a GEO cluster",
          );
      } catch (error) {
  ...
  makeChart(labelNew: string) {
    ...
    this.sortingSettings = SettingsPanelFactory.add(
      ClusterFactory.getVClusterByLabel(labelNew),
      false,
      chartHeader,
    );
  ...
  private makeSVG(): SVGElement {
    ...
    let groupContainer = document.createElementNS(
    ...
    for (let i = 0; i < this.items.length; i++) {
      ...
      let group = this.items[i].makeBarGroup(
        xStep,
        yPos,
        i,
        this.minValue!,
  ...
  private runSorting(settings: ClusterSettings) {
    // get the selected value
    const userSelection = settings.getDimensionMap();

    if (!userSelection || userSelection.size === 0) {
      // nothing selected, abort sorting
      return;
    }

    // Get the selected value from the sorting widget dropdowns
    const firstChoice = [...userSelection].at(0);
    const firstKey = firstChoice![0];
    const firstValue = firstChoice![1];
    const lastChoice = [...userSelection].at(-1);
    const lastKey = lastChoice![0];
    const valueKey = lastChoice![1];
    const periodSelected = settings.getYearControl().value; // Primera_Vuelta
    ...
    if (firstChoice && lastChoice && periodSelected) {
      for (let i = 0; i < this.items.length; i++) {
        ...
        try {
          ...
          console.warn(
            "item at " + i + " does not have " + this.label + " data",
          );
        }

        let index = 0;

        // This lookupTable is loaded from the JSON file.
        const dataLookupTable: Object | undefined =
          this.sortingSettings?.getVCluster().cluster.lookupTable;

        if (dataLookupTable) {
          // Ensure we iterate over entries of the lookup table (it's an object, not an iterable)
          for (const entry of Object.entries(dataLookupTable)) {
            // entry is [key, value]
            if (periodSelected == entry[0]) index = entry[1];
            break;
          }
        }
        ...
        if (attributes) {
          ...
          let data = Object.values(attributes)[index] as
            | Record<string, unknown>
            | unknown[];

          // make data an object if it is not
          if (Array.isArray(data)) {
            data = { ...data };
          }

          const dataObject = data as Record<string, unknown>;
          if (Object.hasOwn(dataObject, valueKey)) {
            attrValue = dataObject[valueKey];
            this.currentPalette = ColorFactory.getSequentialPalette(valueKey);
          } else {
            for (const elementKey in dataObject) {
              const elementValue = dataObject[elementKey];
              if (
                elementValue &&
                typeof elementValue === "object" &&
                Object.hasOwn(elementValue as object, firstKey)
              ) {
                const elementValueObject = elementValue as Record<
                  string,
                  unknown
                >;
                if (elementValueObject[firstKey] == firstValue) {
                  attrValue = elementValueObject[valueKey];
                  this.currentPalette =
                    ColorFactory.getSequentialPalette(valueKey);
                }
              }
            }
          }
        ...
        if (attrValue !== undefined) {
          item.setValue(NaN); // Reset the value to 0 before assigning a new value

          // console.log(`Item ${item.label} in ${selectedValue} has value: ${attrValue}`);
          // Determine if attrValue can be casted to a number
          if (!isNaN(Number(attrValue))) {
            //console.log('before =' + item.value);
            item.setValue(Number(attrValue));
